    ...
    axios
      ...
      .then(
        axios.spread((pendingResponse, acceptResponse) => {
          console.log(
            'Data dari API sebelum dispatch:',
            pendingResponse.data,
            acceptResponse.data,
          ); // Log data dari kedua API sebelum dispatch
          ...
          const acceptData = Array.isArray(acceptResponse.data.data)
            ...
            : [];
          const combinedData = [...pendingData, ...acceptData];
...
export const getPastBooking = () => (dispatch) => {
    getData('token').then(resToken => {
        ...
        axios.all([cancelledRequest, declineRequest])
            .then(axios.spread((cancelledResponse, declineResponse) => {
                ...
                const combinedData = [...cancelledData, ...declineData];
